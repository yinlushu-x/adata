...
def multi_thread_market(max_workers=5):
    """
    多线程获取股票行情数据的装饰器
    ...
    def decorator(func):
        @wraps(func)
        def wrapper(self, stock_code=None, start_date='1990-01-01', end_date=None, k_type=1, adjust_type=1):
            # 判断是否为单个股票代码
            if isinstance(stock_code, str):
                # 单个股票，直接调用原函数
                return func(self, stock_code, start_date, end_date, k_type, adjust_type)
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                    # 提交所有任务
                    future_to_stock = {
                        executor.submit(func, self, code, start_date, end_date, k_type, adjust_type): code
                    ...
                    for future in concurrent.futures.as_completed(future_to_stock):
                        ...
                        try:
                            ...
                            print(f'股票 {stock} 获取数据时发生异常: {exc}')
                            # 添加空的DataFrame以保持顺序
...
class StockMarket(object):
    ...
    def get_market(self, stock_code: str = '000001', start_date='1990-01-01', end_date=None, k_type=1,
        ...
        :return: k线行情数据，如果是多个股票则返回合并后的DataFrame
        """
...
if __name__ == '__main__':
    # 测试单个股票
    print("单个股票测试:")
    single_stock = StockMarket().get_market(stock_code='002230', start_date='2024-07-22', k_type=1)
    print(single_stock.head())
    
    # 测试多个股票
    print("\n多线程获取多个股票测试:")
    multi_stocks = StockMarket().get_market(stock_code=['000001', '600001', '000795'], 
                                            start_date='2024-07-22', k_type=1)
